...
df = pd.read_excel('song_mid民谣.xlsx')
data = df.iloc[:,[0]]
...
file_name = '.\\song_info_民谣1.xlsx'
workbook = xlsxwriter.Workbook(file_name)
worksheet = workbook.add_worksheet('first_sheet')
worksheet.write_row('A1',['name','song_mid','url','img','lyric','singer_mid'])
for i in range(len(data)):
    try:
        ...
        worksheet.write_row(f'A{i+2}',[f'{song_name}-{singer_name}',data[i][0],song_url,song_img,song_lyric,singer_mid])
        print(f'{i+2}')
